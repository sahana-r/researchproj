...
def populate_dict_for_db(dict_for_db, image, boxes, classes, scores, 
                        category_index, image_id=None, min_score_thresh=.5, 
                        max_boxes=20):
    """Populates a given dictionary with values to input into the database.
    Inputs: 
        dict--the dict to populate
        image--numpy array representation of the image
        boxes--numpy array representing N bounding boxes; each element
            is made up of (ymin, xmin, ymax, xmax)
        scores--numpy array of scores for each box
        category_index--dictionary of categories
        image_id--optional parameter representing image's unique ID 
            that we use to map it into the database
        min_score_thresh--we want scores to be higher than this 
    ...
    for i in range(min(max_boxes, boxes.shape[0])):
        ...
        if image_id:
            ...
            if [box_height, box_width, box_xmin, box_ymin, this_label, scores[i]] not in dict_for_db[image_id]:
                dict_for_db[image_id].append([float(box_height), float(box_width), 
                    float(box_xmin), float(box_ymin), this_label, float(scores[i])])
                # dict_for_db[image_id] += [[float(box_height), float(box_width), 
                #     float(box_xmin), float(box_ymin), this_label, float(scores[i])]]


#transform box mask to image size
def mask_transform(box_masks, boxes, img_height, img_width):
    """Transforms box masks to image size.
    Inputs:
        box_masks: the box masks
        boxes: the box tensor
    ...
    img_masks = tf.image.crop_and_resize(box_masks,reverse_boxes,
        tf.range(num_boxes), [img_height, img_width])
